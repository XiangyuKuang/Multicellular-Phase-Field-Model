...
for SimIdx = 1:SimNum
	rng(SimIdx)
  SimName = sprintf('sn%s_%d',dot2d(noise),SimIdx);
  SimStageEnd = StageEnd;
  SimStageEnd(8).SimParaModify.Noise = noise;
  mainBDF2VC_contact(SimName,L,N,dt,DefaultSimParameters,CellParameters,SimStageEnd,tspan,...
...
% zeros noise, identical to simulation 8adhto12dx0d25dt0d1c18sigma0d5-0_3
noise = 0;
SimName = sprintf('sn%s_%d',dot2d(noise),1);
SimStageEnd = StageEnd;
SimStageEnd(8).SimParaModify.Noise = noise;
mainBDF2VC_contact(SimName,L,N,dt,DefaultSimParameters,CellParameters,SimStageEnd,tspan,...
  'EggshellConfig',eggshellConfig,...
...
load('sn0_1/sn0_1-IC_34700.mat','phi_save','CellIdx')
[contact_area, ~] = ContactArea(phi_save,dr,th);
[~,I] = sort(CellIdx);
contact_area_sort_IC = contact_area(I,I);
save('sn0_1/IC_contact_area.mat','contact_area_sort_IC')