...
for r=1:20000
Maxcelltype(r,1)=max(signal_pathway{r, 5}.cell_types);
end

% 找到 Maxcelltype 中所有值为 7 的元素的位置
indices_7 = find(Maxcelltype == 7);
count_7 = numel(indices_7);  % 统计 7 的数量

%最终命运数为8的命运分布
seven_cell_type = signal_pathway(indices_7,:); 
final_seven_cell_type = cell(size(seven_cell_type));  
for i = 1:size(seven_cell_type, 1)
    for col=1:5
    final_seven_cell_type{i,col} = signal_pathway{i,col}.cell_types;  % 提取第6列的 cell_types 字段
    end
end
