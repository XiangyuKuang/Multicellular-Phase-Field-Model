...
%将该命运整理为从1开始
for r=1:size(final_five_cell_type,1)
    for c=1:size(final_five_cell_type,2)
        Cell_types = final_five_cell_type{r,c};
        ...
        final_five_cell_type{r,c}=sorted_cell_types;
    end
end

new_array = {};
indices = [];
for i = 1:size(final_five_cell_type, 1)
    current_value = final_five_cell_type{i, 6};

    % 检查该值是否与其他行的第 6 列值相同
    is_unique = true; 
    for j = 1:size(final_five_cell_type, 1)
        % 如果 i != j 并且当前值与其他行的第 6 列值相同
        if i ~= j && all(current_value == final_five_cell_type{j, 6})
            is_unique = false;  
            break;
        end
    end
    ...
    if is_unique
        new_array = [new_array; final_five_cell_type(i, :)]; 
        indices = [indices, i];
    end
...
asymmetric_400_allsignal = cell(20,6);  
asymmetric_400_allsignal(1:20, :) = new_array(1:20, :);