...
load('E:\data\cell_contact.mat');
load('E:\data\color_cell_type.mat');
load('E:\data\embryo.mat');
...
for A=2%选择细胞期
    ...
    %[v, x, y, z, t_stage] = calculateRXYZ_3cellstage(C,Embryo);
    num_cells = length(C);
    load(['E:\code\3D cell contact\', num2str(num_cells), '_t.mat']);
    load(['E:\code\3D cell contact\', num2str(num_cells), '_x.mat']);
    load(['E:\code\3D cell contact\', num2str(num_cells), '_y.mat']);
    load(['E:\code\3D cell contact\', num2str(num_cells), '_z.mat']);
    load(['E:\code\3D cell contact\', num2str(num_cells), '_v.mat']);
    %将坐标移动到0附近
    x0 = x - mean(x, 2);
    z0 = y - mean(y, 2);
    y0 = -z + mean(z, 2);
    ...
    for i = 1:length(C)
        ...
        set(h(i), 'AmbientStrength', 0.5, 'DiffuseStrength', 0.6, 'SpecularStrength', 0.4);
    end
    ...
    %坐标轴设置&图例
    legend(h, C);
    set(legend,'unit','centimeters','position',[14.7,6.4,2,0.525*length(C)]);
    set(legend, 'Box', 'off');
    ...
    filename1 = sprintf('cell_type_%d.png',length(C));
    ...
    text(4,2.8,sprintf('%.f', (maxM_1-minM_1)./3*2+minM_1),'FontSize',14,'Fontname', 'Arial');
    text(-6,2.8,sprintf('%.f', (maxM_1-minM_1)./3*1+minM_1),'FontSize',14,'Fontname', 'Arial');
    text(16.9,3.2,'{\it(μm^2)}','FontSize',14,'Fontname', 'Arial');

    grid off;
    axis equal;
    axis off;

    filename2 = sprintf('cell_type_%d_1.png',length(C));
