function SimReport = mainBDF2VC_phinoise(SimName,L,N,h,seed,DefaultSimParameters,CellParameters,StageEnd,TimeSpan,varargin)
...
%            T|           |     |           |
%     TimeCost|           |     |           |
sg = parallel.gpu.RandStream('Threefry','NormalTransform','Inversion','Seed',seed);
...
%   vnoiserdt = SimParameters.vnoise*sqrt(h);
  phinoiserdt = SimParameters.phinoise*sqrt(h);
  ...
  for n = 1:CellNum
%     ut = vnoiserdt*randn(3,1);% v noise
    PhiNoise = phinoiserdt*randn(N([2,1,3]),'gpuArray');% phi noise
    AtrX = sigma1(n)*(PhiXSum-PhiXStack{n});%+ut(1);
    AtrY = sigma1(n)*(PhiYSum-PhiYStack{n});%+ut(2);
    AtrZ = sigma1(n)*(PhiZSum-PhiZStack{n});%+ut(3);
    ...
    fF = fftn( (MRV(n)+PhiNoise).*sqrt(PhiXStack{n}.^2+PhiYStack{n}.^2+PhiZStack{n}.^2)...%volume constraint
